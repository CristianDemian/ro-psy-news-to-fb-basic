...
async function callOpenAI(system, user, model = 'gpt-4o-mini') {
...
Actor.main(async () => {
  ...
  const selectorSystem = 'Ești un editor pentru un cabinet de psihoterapie. Alege 3 știri relevante psihologic și întoarce doar numerele lor separate prin virgulă.';
  const rawChoice = await callOpenAI(selectorSystem, listPrompt, openaiModel);
  const indexes = rawChoice.match(/\d+/g)?.map(n => parseInt(n, 10))?.slice(0,3) || [];

  const dataset = await Actor.openDataset();
  for (const idx of indexes) {
    const it = items[idx - 1];
    if (!it) continue;
    const postSystem = 'Scrii o postare Facebook de ' + postWordTarget + ' cuvinte, ton cald, clar.' +
      (includeCTA ? ' Încheie cu: ' + brandCTA : '');
